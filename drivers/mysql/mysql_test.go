...
func init() {
	// 注册驱动
	err := dbhelper.RegisterDriver(mysql.DriverName, mysql.GetDriver())
	if err != nil {
		return
	}
}

func TestCondBuilder_MySQL(t *testing.T) {
	driver, err := dbhelper.GetDriver(mysql.DriverName)
	if err != nil {
	...
	// 测试Exec条件
	execCond := dbhelper.Cond().Raw("CREATE TABLE test (id INT PRIMARY KEY AUTO_INCREMENT, name VARCHAR(32))").Build()
...
func TestMySQLDriver_CRUD(t *testing.T) {
	...
	createTable := dbhelper.Cond().Raw("CREATE TABLE IF NOT EXISTS user (id INT PRIMARY KEY AUTO_INCREMENT, name VARCHAR(32), age INT)").Build()
